  ...
  if (!dryRun) {
    await prisma.pftSetting.upsert({
      ...
      update: {
        ...
        defaultOtherSources: pick('default_other_sources'),
        defaultRent: pick('default_rent'),
        defaultCook: pick('default_cook'),
        defaultLoanRepayment: pick('default_loan_repayment'),
        defaultSipMf: pick('default_sip_mf'),
        defaultStocks: pick('default_stocks'),
        defaultFd: pick('default_fd'),
        defaultSavings: pick('default_savings'),
    ...
    if (!dryRun) {
      await prisma.monthlyPlan.upsert({
        ...
        update: {
          ...
          sipMf: toNumber(row.data.sipMf ?? row.data.sip),
          ...
          stocks: toNumber(row.data.stocks ?? row.data.investment),
          fd: toNumber(row.data.fd),
          savings: toNumber(row.data.savings ?? row.data.liquidSaved ?? row.data.liquid_saved),
          ...
          otherSources: toNumber(row.data.otherSources ?? row.data.otherIncome ?? row.data.other_income),
